...
function groupBy($array, $keys)
{
    ...
    foreach ($array as $item) {
        // Kalitlarni yaratadi: bir xil qiymatlarni aniqlash uchun
        $groupKey = [];
        foreach ($keys as $key) {
            $groupKey[] = $item[$key];
        }
 ...
 * @return string
 */
function imageUrlToBase64Curl($url) {
    $ch = curl_init($url);
...
function getTokenIIP() {
    $cookies = \Yii::$app->response->cookies;
    if ($cookies->has('IIPTokenName')) {
        return $cookies->getValue('IIPTokenName');
    } else {
        $client = new Client();
        $url = "https://ejournal.uzbmb.uz/davomat/api/auth/login/";
        $data = [
            "username" => "face_user",
            "password" => "Uzbmb123"
        ];
...
//            prd($response->data['result']['access_token']);

            if ($response->getIsOk()) {
                $cookies->add(new \yii\web\Cookie([
                    'name' => 'IIPTokenName',
                    'value' => $response->data['result']['access_token'],
                    'expire' => time() + 3600,
                ]));
...
function langs() {
    $lang = '_uz';
    if (user()->identity->lang == 'ru')
        $lang = '_ru';
    elseif (user()->identity->lang == 'qq')
        $lang = '_kk';
    return $lang;
}

function dateTime($d_time) {
...
// API response
function api_response($status = 0, $message = null, $data = null, $errors = null, $responsStatusCode = null) {
    if (is_null($message)) {
        $message = _e('So‘rovingizni ko‘rib chiqishda xatolik yuz berdi.');
    } elseif ($message === true) {
        $message = _e('Success');
    } elseif ($message === false) {
        $message = _e('Results not found.');
    }

    if (is_numeric($responsStatusCode)) {
...
// Translation app
function _e($message, $params = array(), $language = null) {
    $translation = \Yii::t('app', $message, $params, $language);

    try {
        return ($translation != $message) ? $translation : \Yii::t('system', $message, $params, $language);
    } catch (\Exception $e) {
        return $translation;
    }
}
...
{
    return date_format(date_create($input),"Y-m-d");
}
...
function pr_echo($variable) {
    echo($variable);
    die;
}

function psnum($psnum) {
    return str_pad($psnum, 7, 0, STR_PAD_LEFT);
}
