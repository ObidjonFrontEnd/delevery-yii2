            ...
            <?php if(isset($restauran) && !empty($restauran) ):?>
                <div class="bg-white dark:bg-[#1f1f1f] shadow-md rounded-lg overflow-hidden">
                    ...
                    <div class="h-48 w-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
                        <?php if (!empty($restauran->image)): ?>
                            <img src="<?= Yii::getAlias('@web') . '/image/' . htmlspecialchars($restauran->image) ?>"
                                 alt="<?= htmlspecialchars($restauran->title) ?>"
                    ...
                    <div class="p-6">
                        <h2 class="text-2xl font-semibold mb-2 capitalize">
                            <?= htmlspecialchars($restauran->title ?? 'Без названия') ?>
                        </h2>
                        ...
                        <div class="mt-4 flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <span class="text-lg ml-1"><?= $restauran->rate ?? '0.0' ?></span>
                            </div>

                            <div class="text-lg">
                                <?= htmlspecialchars($restauran->phone_number ?? 'Не указан') ?>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                            <?php foreach ($category['products'] as $product): ?>
                                <div class="bg-white dark:bg-[#1f1f1f] shadow-md rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
                                    <!-- Изображение продукта с фиксированной высотой -->
                                    <div class="relative h-48 w-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
                                        ...
                                        <?php else: ?>
                                            <div class="w-full h-full flex items-center justify-center bg-gray-300 dark:bg-gray-600">
                                                <i class="bx bx-dish text-6xl text-gray-400 dark:text-gray-500"></i>
                                            </div>
                                        <?php endif; ?>
                                    ...
                                    <div class="p-4 flex flex-col min-h-[200px]">
                                        <h3 class="text-lg font-semibold mb-2 capitalize line-clamp-2">
                                            <?= htmlspecialchars($product['name'] ?? 'Без названия') ?>
                                        </h3>
                                        <p class="text-gray-700 dark:text-white mb-2">
                                            Narx: <span class="font-bold">
                                            <?= ($product['price'] ? number_format($product['price'], 0, '.', ' ') : "0") ?> UZS</span>
                                        </p>
